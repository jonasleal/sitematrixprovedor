<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Editar Aplicativo / Download</h2>
    </x-slot>

    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white p-6 shadow-sm sm:rounded-lg" x-data="{ 
                links: {{ json_encode($download->links->map(function($l) {
                    return [
                        'id' => $l->id,
                        'plataforma' => $l->plataforma,
                        'tipo' => filter_var($l->link, FILTER_VALIDATE_URL) ? 'externo' : 'upload',
                        'link_externo' => filter_var($l->link, FILTER_VALIDATE_URL) ? $l->link : '',
                        'arquivo_atual' => filter_var($l->link, FILTER_VALIDATE_URL) ? '' : $l->link
                    ];
                })) }},
                adicionarLink() {
                    this.links.push({ id: null, temp_id: Date.now(), plataforma: 'android', tipo: 'externo', link_externo: '', arquivo_atual: '' });
                },
                removerLink(index) {
                ...
                <form action="{{ route('admin.downloads.update', $download->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-4">
                        <div>
                            <x-input-label for="titulo" value="Título Exibido" />
                        ...
                        <div>
                            ...
                            <select name="categoria" id="categoria" class="w-full mt-1 border-gray-300 rounded-md shadow-sm">
                                <option value="aplicativo" {{ $download->categoria == 'aplicativo' ? 'selected' : '' }}>Aplicativos e Softwares</option>
                    ...
                    <div class="mb-4 p-4 bg-gray-50 border border-gray-200 rounded-lg flex items-center space-x-6">
                        ...
                        @endif
                        <div class="flex-grow">
                            <x-input-label for="imagem" value="Atualizar Logo / Ícone (Opcional - Serás convertido para WebP)" />
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-4">
                        <div>
                            <x-input-label for="versao" value="Versão Geral" />
                    ...
                    <template x-for="(link, index) in links" :key="link.id || link.temp_id">
                        <div class="relative p-4 mb-4 bg-gray-50 border border-gray-200 rounded-lg shadow-sm">
                            <button type="button" @click="removerLink(index)" class="absolute top-2 right-2 text-red-500 hover:text-red-700">
                                <i class="fas fa-times-circle text-lg"></i>
                            </button>

                            <template x-if="link.id">
                                <input type="hidden" x-bind:name="'links[' + index + '][id]'" :value="link.id">
                            </template>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mt-2">
                                ...
                                <div class="mt-1">
                                    <div x-show="link.tipo === 'externo'">
                                        ...
                                        <x-text-input x-bind:name="'links[' + index + '][link_externo]'" x-model="link.link_externo" type="url" placeholder="https://..." class="w-full mt-1 text-sm" />
                                    </div>
                                    <div x-show="link.tipo === 'upload'">
                                        ...
                                        <template x-if="link.arquivo_atual">
                                            <p class="text-xs text-gray-500 mt-1">Atual: <code x-text="link.arquivo_atual"></code></p>
                                        </template>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div class="flex justify-end space-x-3 border-t pt-4">
                        ...
                        <x-primary-button>Atualizar Download Completo</x-primary-button>
                    </div>
                </form>
            </div>
        </div>
